...
class DeliveryCarrierFile(models.Model):
    ...
    def get_type_selection(self):
        ...
        result.append(('dhl_versenden', 'DHL Versenden'))
        return result

    type = fields.Selection(get_type_selection, 'Type', required=True)
    billing_number = fields.Char('Billing Number')
    dhl_package_type = fields.Many2one('delivery.dhl.package.type', string="dhl packaging code")
